    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-8">
          <div>
            <h1 className="text-[24px] md:text-[32px] font-extrabold mb-1 md:mb-2 text-slate-900">Product Management</h1>
            <p className="text-slate-500 text-sm md:text-base font-medium">Add, edit and manage your product catalog</p>
          </div>
          <button
          ...
          <AnimatePresence>
            {displayedProducts.map((product, i) => (
              <motion.div
                ...
                className={`group relative bg-white rounded-[24px] overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.12)] transition-all duration-300 border-2 ${
                  product.stock > 0 ? 'border-transparent hover:border-accent-primary/30' : 'border-red-100 hover:border-red-400/50'
                }`}
              >
                {/* Image Section */}
                <div className="relative h-56 w-full bg-slate-100 overflow-hidden">
                  ...
                  {/* Stock Badge */}
                  <div className={`absolute top-4 right-4 px-3 py-1 rounded-full text-xs font-bold shadow-lg border border-white/20 backdrop-blur-md ${
                ...
                <div className="p-5">
                  ...
                  <h3 className="text-lg font-extrabold text-slate-900 mb-2 truncate" title={product.name}>{product.name}</h3>
                  <div className="flex items-center justify-between mt-3">
                    ...
                    <div className="bg-slate-100 px-3 py-1.5 rounded-lg text-slate-600 font-bold text-sm">
                      Size: {product.size}
                    </div>
                  </div>
                </div>
